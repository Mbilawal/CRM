...
function initDataTable(e, t, a, i, n, s) {
    ...
    if(e == '.table-invoices-single-client'){
        ...
        var date_from = $('input[name="date_from_client"]').val();
        var date_to = $('input[name="date_to_client"]').val();
        ...
        var m = {
            ...
            ajax: {
                ...
                type: "POST",
                data: function(e) {
                    for (var t in "undefined" != typeof csrfData && (e[csrfData.token_name] = csrfData.hash), n)
                        e[t] = $(n[t]).val();}
            },
            buttons: get_datatable_buttons(o)
        };
...
function delete_lead_attachment(t, e, a) {
    confirm_delete() && requestGetJSON("leads/delete_attachment/" + e + "/" + a).done(function(e) {
        !0 !== e.success && "true" != e.success || $(t).parents(".lead-attachment-wrapper").remove()
    }).fail(function(e) {
        alert_float("danger", e.responseText)
    })
}
...
function delete_lead_note(t, e, a) {
    confirm_delete() && requestGetJSON("leads/delete_note/" + e + "/" + a).done(function(e) {
        !0 !== e.success && "true" != e.success || $(t).parents(".lead-note").remove()
    }).fail(function(e) {
        alert_float("danger", e.responseText)
    })
}
...
function recalculate_checklist_items_progress() {
    var e = $('input[name="checklist-box"]:checked').length,
        ...
        a = 0,
        i = $(".task-progress-bar");
    if (0 == t ? ($("body").find(".chk-heading").remove(), $("#task-no-checklist-items").removeClass("hide")) : $("#task-no-checklist-items").addClass("hide"), !(2 < t)) return i.parents(".progress").addClass("hide"), !1;
...
function add_item_to_table(n, s, e, t) {
    if ("" !== (n = void 0 === n || "undefined" == n ? get_item_preview_values() : n).description || "" !== n.long_description || "" !== n.rate) {
        var p = "",
            _ = $("body").find("tbody .item").length + 1;
        p += '<tr class="sortable item" data-merge-invoice="' + e + '" data-bill-expense="' + t + '">', p += '<td class="dragger">', isNaN(n.qty) && (n.qty = 1), ("" === n.rate || isNaN(n.rate)) && (n.rate = 0);
        var o = n.rate * n.qty,
...
function delete_item(e, t) {
    ...
    }), 0 < $('input[name="isedit"]').length && $("#removed-items").append(hidden_input("removed_items[]", t))
}
...
function merge_field_format_url(e, t, a, i) {
    return -1 < e.indexOf("{") && -1 < e.indexOf("}") ? e = "{" + e.split("{")[1] : -1 < e.indexOf("%7B") && -1 < e.indexOf("%7D") && (e = "{" + e.replace("%7B", "").replace("%7D", "") + "}"), e
}
...
$("body").on("loaded.bs.select change", "select.ajax-search", function(e) {
    ...
    side_bar.metisMenu(), setup_menu.metisMenu(), $(".hide-menu").click(function(e) {
        e.preventDefault(), $("body").hasClass("hide-sidebar") ? $("body").removeClass("hide-sidebar").addClass("show-sidebar") : $("body").removeClass("show-sidebar").addClass("hide-sidebar"), setup_menu.hasClass("display-block") && $(".close-customizer").click(), delay(function() {
    ...
    if (s && $("#search_input").val(s).trigger("keyup"), $("body").on("blur", "#timesheet_duration", function() {
        ...
        calendar_selector.fullCalendar(o), get_url_param("new_event") && ($("input[name='start'].datetimepicker").val(get_url_param("date")), $("#newEventModal").modal("show"))
    }
    ...
    if ((table_leads = $("table.table-leads")).length) {
        var c = table_leads.find("#th-consent"),
            p = [0],
            _ = [0, table_leads.find("#th-assigned").index()];
        0 < c.length && (p.push(c.index()), _.push(c.index())), _table_api = initDataTable(table_leads, admin_url + "leads/table", _, p, r, [table_leads.find("th.date-created").index(), "desc"]), _table_api && 0 < c.length && _table_api.on("draw", function() {
    ...
    if ($("body").on("change", 'input[name="contacted_today"]', function() {
        ...
        var m = [];
        m.activity_log_date = '[name="activity_log_date"]', initDataTable(table_activity_log, window.location.href, "undefined", "undefined", m, [1, "desc"])
    }

    var customer_client_id = $('input[name="userid"]').val();

    if(customer_client_id == ''){
        var invoices_table = 'invoices/table';
    }else{
        var invoices_table = 'invoices/table/'+customer_client_id;
    }

    if (table_invoices = $("table.table-invoices"),
        table_estimates = $("table.table-estimates"),
        0 < table_invoices.length || 0 < table_estimates.length) {
        var u = {},
            f = $("._hidden_inputs._filters input");
        ...
        table_invoices.length && initDataTable(table_invoices, admin_url + invoices_table + ($("body").hasClass("recurring") ? "?recurring=1" : ""), "undefined", "undefined", u,
                $("body").hasClass("recurring") ? [table_invoices.find("th.next-recurring-date").index(), "asc"] : [
                    [3, "desc"],
                    [0, "desc"]
                ]),
            table_estimates.length && initDataTable(table_estimates, admin_url + "estimates/table", "undefined", "undefined", u, [
                [3, "desc"],
                [0, "desc"]
            ])
    }
    if ((table_tasks = $(".table-tasks")).length) {
        var h, v = {};
        h = $("._hidden_inputs._filters._tasks_filters input"), $.each(h, function() {
            v[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]'
        });
        var b = [0],
            g = admin_url + "tasks/table";
        $("body").hasClass("tasks-page") && (g += "?bulk_actions=true"), _table_api = initDataTable(table_tasks, g, b, b, v, [table_tasks.find("th.duedate").index(), "asc"]), _table_api && $("body").hasClass("dashboard") && _table_api.column(5).visible(!1, !1).column(6).visible(!1, !1).columns.adjust()
    }
    ...
    var k = $(".todos-sortable");
    (0 < k.length && (k = k.sortable({
        connectWith: ".todo",
        items: "li",
        handle: ".dragger",
        appendTo: "body",
    ...
    $("body").on("submit", "._transaction_form", function() {
        ...
        }), !1) : e.length && 0 === e.find(".orderItem").length ? (e.before('<div class="alert alert-warning mbot20" id="items-warning">' + app.lang.no_items_warning + "</div>"), $("html,body").animate({
            scrollTop: $("#items-warning").offset().top
