...
class Authentication extends App_Controller
{
    ...
    {
        $this->load->view('authentication/login_franchise');
    }
	...
	public function login_merchant(){
		...
		if($email=="" || $password=="" ){	
			$this->session->set_flashdata('err_message', '- Email address or Password is empty. Please try again!');
			redirect(base_url().'admin/authentication/franchise/');	
		}else{
				
			$chk_isvalid_user = $this->Authentication_model->validate_credentials($this->input->post('email'),$this->input->post('password'));
			
			if($chk_isvalid_user){
				 $user_data = [
					'staff_user_id'   => $chk_isvalid_user['staffid'],
					'franchise'   => 1,
	...
	}//end public function login_merchant()
