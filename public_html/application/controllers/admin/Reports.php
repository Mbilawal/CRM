...
class Reports extends AdminController
{
	...
	public function revenue_report(){
		...
		$data["revenue"] = $this->reports_model->revenue_report($today);
		$data["title"] = "Revenue Report";
		$this->load->view("admin/reports/revenue", $data);
	}
	
	public function revenue_report_ajax(){
		$this->load->model("mod_common");
		$data['cities'] = $this->mod_common->get_all_records("tblorders", "DISTINCT(city)", 0, 0, array(), "city", "ASC");
		$data["revenue"] = $this->reports_model->revenue_report($this->input->post());
		$data["title"] = "Revenue Report";
		$this->load->view("admin/reports/revenue_ajax", $data);
	}
    
	public function export_revenue_csv()
     {
         $file_name = 'Revenue_Report_'.date('Ymd').'.csv'; 
         ...
         $header = array("City","Number of Orders","Total Sales","Total Comission Earned","Total Booking Fee","Total Delivery Fee","Total Tips Payed to the Driver"); 
         fputcsv($file, $header);
	...
	public function top_10_drop_up_location_suburb(){
		$data["title"] = "Top 10 Drop Up Location Suburb";
		$this->load->view("admin/reports/top_10_drop_up_location_suburb", $data);
	}
	
	public function top_10_drop_up_location_suburb_ajax(){
		$this->load->model("mod_common");
		$data["top_10_drop_up_locations_by_revenue"] = $this->reports_model->top_10_drop_up_locations_by_revenue($this->input->post());
		$data["top_10_drop_up_locations_by_orders"] = $this->reports_model->top_10_drop_up_locations_by_orders($this->input->post());
		$data["top_10_stores"] = $this->reports_model->top_10_stores($this->input->post());
		$data["top_10_customers_by_revenue"] = $this->reports_model->top_10_customers_by_revenue($this->input->post());
		$data["top_10_customers_by_orders"] = $this->reports_model->top_10_customers_by_orders($this->input->post());
		
		$data["title"] = "Top 10 Drop Up Location Suburb";
		$this->load->view("admin/reports/top_10_drop_up_location_suburb_ajax", $data);
	}
}
