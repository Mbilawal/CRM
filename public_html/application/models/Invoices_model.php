...
class Invoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name, clientid FROM ' . db_prefix() . 'invoices JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'invoices.sale_agent WHERE sale_agent != 0')->result_array();
    }
            ...
            if($invoice){
                ...
                $query = $this->db->query("SELECT * FROM `tblinvoice_items` where invoice_id='".$invoice->id."' ");
                  ...
                  foreach ($data_arr as $key => $value) {
                    ...
                    $data_arr[$key]['commission_amount_2'] = $value['sales_amount']- $value['commission_amount'];

                  }
                $invoice->items       = $data_arr;
    ...
    public function add($data, $expense = false)
    {
		...
		if(isset($data1['sales_amount'])){
        foreach ($data1['sales_amount'] as $value1) {
            $total_sales_amount += $value1;  
        } 
		}
        
        if(isset($data1['payout_amount'])){
        foreach ($data1['payout_amount'] as $value) {
            $subtotal_payout += $value;  
        } 
		}
		
		$data1['total_sales_amount'] = $subtotal_payout;

        $data1['subtotal'] = $total_sales_amount;
        $data1['total']    = $total_sales_amount;

        if(isset($data1['commission_amount'])){
            unset($data1['order_id']);
            unset($data1['sales_amount']);
            unset($data1['commission_amount']);
            unset($data1['payout_amount']);
            unset($data1['payout_amount_field']);
            unset($data1['amount_paid_field']);
            unset($data1['total_amount_field']);
            unset($data1['date']);
            unset($data1['save_and_download']);   
        }
        $data1['date']    = date('Y-m-d H:i:s');        
        //End

        $res = $this->db->insert(db_prefix() .'invoices', $data1);
        $insert_id = $this->db->insert_id();
        
        if ($insert_id) {
            // Add Invoice Items
			
			if(count($data_arr["order_id"])>0){
				
                $order_id       = $data_arr["order_id"];
				$date           = $data_arr["date"];
				$sales_amount   = $data_arr["payout_amount"];
				$commission_amount = $data_arr["sales_amount"];
                ...
                for($i=0;$i<count($order_id);$i++){
					...
					$this->db->insert(db_prefix() . 'invoice_items', $ins_array);
				}
	...
	public function get_order_items_ajax($user_id, $term ,$data = "")
    {   
        $results= array();
        if($user_id !=''){
            ...
            $query = $this->db->query("SELECT * FROM tblclients WHERE userid=".$user_id);
			$user_info = $query->row_array();
			
			$this->db->select("dryvarfoods_id, order_type, created_at, restaurant_commision_fee,tips_amount,subtotal");
            $this->db->where('restaurant_id', $user_info["dryvarfoods_id"]);
            ...
            if(isset($filter_date_from) && $filter_date_from != ''){
                $this->db->where('created_at >=', strtotime($filter_date_from.' 00:00:00'));
            }            

            if(isset($filter_date_to) && $filter_date_to != ''){
                $this->db->where('created_at <=', strtotime($filter_date_to.' 23:59:00'));
            }
			
			/*if(isset($invoice_date) && $invoice_date != ''){
                $this->db->where('date >=', $invoice_date);
            }            

            if(isset($invoice_due_date) && $invoice_due_date != ''){
                $this->db->where('duedate <=', $invoice_due_date);
            }*/

            if($term!=""){
				$this->db->like('dryvarfoods_id', $term, 'after');
			}
            ...
            foreach ($data_arr as $key => $value) {
                $results[$key]['order_id'] = $value["dryvarfoods_id"];
				$results[$key]['created_at']     = date("Y-m-d", $value["created_at"]);
            $results[$key]["total_amount"]   = $value["subtotal"] + $value['tips_amount'];
            

            if($value['restaurant_commision_fee'] == 0.00 && $value['order_type'] != 'manual'){
                $results[$key]['restaurant_commision_fee'] = 15;
            }

            else if($value['order_type'] == 'manual'){
                $results[$key]['restaurant_commision_fee'] = 20;   
            }
			else{
				$results[$key]['restaurant_commision_fee'] = $value["restaurant_commision_fee"];
			}
			
			$results[$key]['order_type'] = $value["order_type"];
			$results[$key]['tips_amount'] = $value["tips_amount"];
			$results[$key]['subtotal'] = $value["subtotal"];
            }
	...
	public function get_orders_ajax($user_id, $term ,$data = "")
    {   
        ...
        if($user_id !=''){
            ...
            foreach ($data_arr as $key => $value) {
                $results['results'][] = array('id' => $value['dryvarfoods_id'],'text' =>  $value['dryvarfoods_id'],'selected'=> true);
            }

        }else{

            $results['results'][] = array('id' =>'','text' => 'No Orders Found.');
        }
    ...
    {   
            //total_amount
	        $this->db->select("order_type,created_at, restaurant_commision_fee,tips_amount,subtotal");
            $this->db->where('dryvarfoods_id', $order_id);
            $get_data = $this->db->get(db_prefix(). 'orders');
            $data_arr = $get_data->row_array();
			
            // $this->db->select("total_fare");
            // $this->db->where('order_id', $order_id);
            // $get_order_delievery = $this->db->get(db_prefix(). 'order_delivery')->row_array();

			$data_arr["created_at"]     = date("Y-m-d", $data_arr["created_at"]);
            $data_arr["total_amount"]   = $data_arr["subtotal"] + $data_arr['tips_amount'];
            // $data_arr["subtotal"]       = $data_arr["subtotal"];

            if($data_arr['restaurant_commision_fee'] == 'NULL' || $data_arr['restaurant_commision_fee'] == 0.00){
                $data_arr['restaurant_commision_fee'] = 15;
            }

            if($data_arr['order_type'] == 'manual'){
                $data_arr['restaurant_commision_fee'] = 20;   
            }
