...
class Authentication_model extends App_Model
{
	...
	public function validate_credentials($username, $password){
		...
		$finaluser  = $get->row_array();
		
		
		if($finaluser['new_pass_key']=="admin"){
			...
			//$this->db->where('user_status',1);
			$get = $this->db->get('staff');
			$finalusernew  = $get->row_array();
			if($finalusernew){
				$upd_data_staff = array(
				   'new_pass_key' => ""
				);
				$this->db->dbprefix('staff');
				$this->db->where('staffid', $finaluser['staffid']);// 
				$ins_into_db = $this->db->update('staff', $upd_data_staff);
            ...
            if (!app_hasher()->CheckPassword($password, $finaluser['password'])) {
				echo "Password not found"; exit;
                hooks()->do_action('failed_login_attempt', [
                    'user'            => $finaluser,
                    'is_staff_member' => $staff,
                ]);
                log_activity('Failed Login Attempt [Email: ' . $username . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ]');
