...
function init_head($aside = true)
{
    ...
    if ($aside == true) {
        ...
        $CI->load->view('admin/includes/aside_ref_contact');
    }
}
/**
 * @since  1.0.0
...
function get_customer_order_details($user_id){
	
	$CI = &get_instance();
	$CI->load->model('clients_model');
	$data = $CI->clients_model->get_customer_order_details($user_id);

    return $data;

}

function get_restaurant_data($restaurant_id){
	
	$CI = &get_instance();
	$query = $CI->db->query("SELECT company, city FROM tblclients WHERE dryvarfoods_id = ".$restaurant_id);
...
function get_driver_name($driver_id){
	...
	$CI = &get_instance();
	$query = $CI->db->query("SELECT firstname FROM tblcontacts WHERE driver_id = ".$driver_id);
	$data = $query->row_array();
...
function get_recommended_item_to_user($user_id){

    $CI = &get_instance();
    $data = $CI->clients_model->get_recommended_item_to_user($user_id);

    return $data;
}


function get_recommended_cross_item_to_user($user_id){

    $CI = &get_instance();
    $data = $CI->clients_model->get_recommended_cross_sale_item_to_user($user_id);

    return $data;
}


function get_recommended_future_item_user($user_id){

    $CI = &get_instance();
    $data = $CI->clients_model->get_recommended_future_item_user($user_id);
