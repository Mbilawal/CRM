...
array_push($where, 'AND ' . db_prefix() . 'contacts.type =2 ');
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'contacts.id as id']);
...
foreach ($rResult as $aRow) {
    $row = [];
    $url2 = admin_url('clients/orders/driver/' . $aRow['dryvarfoods_id']);
    $rowName = '<img src="' . contact_profile_image_url($aRow['id']) . '" class="client-profile-image-small mright5"><a href="'. $url2 .'" >' . $aRow['firstname'] . '</a>';
