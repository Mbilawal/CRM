...
class Statement_model extends App_Model
{
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        for ($i=0; $i < count($merged); $i++) { 
            ...
            $invoice_id = $merged[$i]['invoice_id'];
            
            $get_item_data = $this->db->query("SELECT order_id from tblinvoice_items where invoice_id='".$invoice_id."'");
            $orders_id_arr = $get_item_data->result_array();

            $ids_arr = array_values(array_unique(array_column($orders_id_arr, 'order_id')));
            $order_id_str = '';
            if(!empty($ids_arr)){
                $order_id_str = implode($ids_arr, ',');
            }

            $merged[$i]['order_ids'] = $order_id_str;
