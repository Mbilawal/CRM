...
// Bill to
$client_info = get_client_address($invoice->clientid);
$country = get_country($client_info["country"]);
$invoice_info = '<b>' . _l('Payment report to') . '</b>';
$invoice_info .= '<br><address><a style="color: #008ece;text-decoration: none;" href="' . admin_url('clients/client/' . $invoice->clientid) . '" target="_blank"><b>'.$client_info["company"].'</b></a><br>'.$client_info["address"].'<br>
	  '.$client_info["city"].", ".$client_info["zip"].'<br>'.$client_info["state"].'<br>'.$country->short_name.'</address>';
...
$invoice_info .= '<br><br />' . _l('invoice_data_date') . ' ' . _d($invoice->date) . '<br />';

if (!empty($invoice->duedate)) {
    $invoice_info .= _l('invoice_data_duedate') . ' ' . _d($invoice->duedate) . '<br />';
}
$invoice_info .= date('Y-m-d',strtotime($invoice->date)-604800).' To '.$invoice->date . '<br />';
...
<tr style="background-color:#f0f0f0;">
    <td align="right" width="85%"><strong>' . _l('Payout Amount') . '</strong></td>
...
if (count($invoice->payments) > 0 && get_option('show_total_paid_on_invoice') == 1) {
    $tbltotal .= '
    <tr>
        <td align="right" width="85%"><strong>' . _l('Amount Paid') . '</strong></td>
...
if (get_option('show_amount_due_on_invoice') == 1 && $invoice->status != Invoices_model::STATUS_CANCELLED) {
    $tbltotal .= '<tr style="background-color:#f0f0f0;">
       <td align="right" width="85%"><strong>' . _l('Amount Due') . '</strong></td>
