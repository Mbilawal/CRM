...
$aColumns = array_merge($aColumns, [
    ...
    db_prefix() . 'contacts.dryvarfoods_id as dryvarfoods_id',
    'title',
    'last_login',
...
$join         = ['JOIN ' . db_prefix() . 'clients_2 ON ' . db_prefix() . 'clients_2.userid=' . db_prefix() . 'contacts.userid'];
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'contacts.id as id', db_prefix() . 'contacts.userid as userid', 'is_primary', '(SELECT count(*) FROM ' . db_prefix() . 'contacts c WHERE  c.userid=' . db_prefix() . 'contacts.userid) as total_contacts', db_prefix() . 'clients_2.registration_confirmed as registration_confirmed']);
...
foreach ($rResult as $aRow) {
    ...
    $row = [];
	$fulname = explode(' ', $aRow['firstname'],2);

    $url2 = admin_url('clients/customer_details/' . $aRow['dryvarfoods_id']);

    $rowName = '<img src="' . contact_profile_image_url($aRow['id']) . '" class="client-profile-image-small mright5"><a href="'. $url2 .'">' . $fulname[0] . '</a>';

    $rowName .= '<div class="row-options">';

    $rowName .= '<a href="#" class="contact_modal" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;">' . _l('edit') . '</a>';
    ...
    $order_details = get_customer_order_details($aRow['dryvarfoods_id']);

    $row[] = $order_details['last_order'];
    $row[] = $order_details['last_30_day_orders'];
    $row[] = $order_details['total_orders'];
    $row[] = $order_details['prefered'];
    $row[] = $order_details['loyalty_amount'];
    $row[] = $order_details['reward_amount'];
