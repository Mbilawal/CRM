...
$aColumns = [
    ...
    'company',
    db_prefix().'clients.crm_record as crm_record',
    db_prefix().'clients.dryvarfoods_id as dryvarfoods_id',
	db_prefix().'clients.user_status as user_status',
    'firstname',
    'email',
...
array_push($where, 'AND ('.db_prefix().'clients.is_archive = 0)');
...
foreach ($rResult as $aRow) {
    ...
    // Company
    $company  = ($aRow['company']=="" || $aRow['company']==" ") ? "<span style='color:red;'>N/A</span>>" : $aRow['company'] ;
    $isPerson = false;
    ...
    $url2 = admin_url('clients/orders/client/' . $aRow['dryvarfoods_id']);

    if ($isPerson && $aRow['contact_id']) {
        $url .= '?contactid=' . $aRow['contact_id'];
    }

    $company = '<a href="' . $url2 . '">' . $company . '</a>';

    $company .= '<div class="row-options">';
    $company .= '<a href="' . admin_url('merchant_dashboard/merchant_profile/' . $aRow['dryvarfoods_id']) . '">' . _l('profile') . '</a>';

    $company .= ' | <a href="' . admin_url('clients/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts' : '')) . '">' . _l('view') . '</a>';

    $company .= ' | <a href="' . admin_url('merchants/report/' . $aRow['dryvarfoods_id']) . '">Report</a>';
    ...
    if ($hasPermissionDelete) {
        $company .= ' | <a class="disabled" href="' . admin_url('clients/delete/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
	
	$company .= ' | <a href="' . admin_url('clients/archive/' . $aRow['userid']) . '" class="text-danger _archive">' . _l('Archive') . '</a>';
	...
	if( $aRow['user_status'] == 1 ){
        ...
        $row[] = '<span class="label label-success">ONLINE</span>';
    }else{
        $row[] = '<span class="label label-danger">OFFLINE</span>';
    }
	
	
	if( $aRow['crm_record'] == 1 ){
    ...
    // Primary contact
    //$row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : 'N/A');

    // Primary contact email
    //$row[] = ($aRow['email'] ? '<a href="mailto:' . $aRow['email'] . '">' . $aRow['email'] . '</a>' : 'N/A');

    // Primary contact phone
    $row[] = ($aRow['phonenumber'] ? '<a href="tel:' . $aRow['phonenumber'] . '">' . $aRow['phonenumber'] . '</a>' : 'N/A');
	...
	$date =  ($aRow['datecreated']);
    
	if(empty($date) || $date==' ' || $date=="0000-00-00"){
      $row[] = 'N/A';
	}else{
	  $dateReturn = _dt($aRow['datecreated']);
	  
	  $row[]  = ($dateReturn) ? $dateReturn : "0000-00-00"; 
	  
	}

    // Custom fields add values
    foreach ($customFieldsColumns as $customFieldColumn) {
		$customFieldColumnIst  = ($aRow[$customFieldColumn]) ? $aRow[$customFieldColumn] : "N/A";
        $row[] = (strpos($customFieldColumn, 'date_picker_') !== false ? _d($aRow[$customFieldColumn]) : $customFieldColumnIst);
    }
