...
$from_date = $this->ci->input->post('date_from');
$to_date   = $this->ci->input->post('date_too');

if($from_date != "" || $to_date != ""){
    
    $start_week = date("Y-m-d",strtotime($from_date));
    $end_week   = date("Y-m-d",strtotime($to_date));
    
    if($from_date != ""){
        array_push($where, "AND tblinvoices.date >= '$start_week' ");
    }

    if($to_date !=""){
        array_push($where, "AND tblinvoices.date <= '$end_week' ");
    }
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    db_prefix() . 'invoices.total_sales_amount',
    db_prefix() . 'invoices.total_orders',
...
foreach ($rResult as $aRow) {
	$total_orders_query = $this->ci->db->query("SELECT count(*) as total_orders FROM tblinvoice_items WHERE invoice_id = ".$aRow['id']);
	if($total_orders_query->num_rows()>0){
	$res = $total_orders_query->row_array();
    ...
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id'].'?clientid='.$clientid).'" target="_blank">'.$aRow['id'].'</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" onclick="init_invoice(' . $aRow['id'] . '); return false;">' . $aRow['id'] . '</a>';
    }
    ...
    $numberOutput .= '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id'].'?clientid='.$clientid) . '" target="_blank">' . _l('view') . '</a>';
    ...
    $row[] = app_format_money($aRow['total_sales_amount'], $aRow['currency_name']);
