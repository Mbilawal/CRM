...
$('body').on('click', '.close', function() {
	...
	$("#contact").modal("hide");
    //code
	//$("#contact_data,#contact").addClass("in");
    //$("#contact_data,#contact").css("display", "block");
	//$("#contact_data:first-child").addClass("in");
	//$(this).closest('#contact_data').find('div#contact').show();
});
...
$(function() {
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    $('.apply_filter_date').on('click', function(e) {
...
function contact(client_id, contact_id) {
    ...
    requestGet('clients/form_contact/' + client_id + '/' + contact_id).done(function(response) {
		...
		$("#contact_data,.modal").addClass("in");
	   $("#contact_data:first-child").addClass("in");
