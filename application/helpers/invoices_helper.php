...
{
    $CI = & get_instance();
    $CI->db->select('id,date,number,prefix,number_format')->from(db_prefix() . 'invoices')->where('id', $id);
...
function get_client_address($id)
{
    ...
    $CI->db->select('company, address, city, state, country, zip');
    $CI->db->from(db_prefix() . 'clients');
    $CI->db->where('userid', $id);
    $invoice = $CI->db->get()->row_array();
    return $invoice;
}
